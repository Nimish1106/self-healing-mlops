...
def check_retraining_needed(**context):
    ...
    try:
        ...
        has_sufficient_data = (
            coverage['labeled_predictions'] >= MIN_LABELED_SAMPLES and
            coverage['coverage_rate'] >= MIN_COVERAGE_PCT
        )
        ...
        if drift_detected:
            ...
            logger.warning(f"  ⚠️ Drift events: {len(drift_details.get('drift_timestamps', []))}")            # OBSERVATION-ONLY MODE: Log detection but no automated action
            logger.warning("=" * 80)
            ...
            logger.info(f"  No significant drift (observed drift_share: {drift_details.get('drift_share', 0.0):.3f})")
        ...
        context['task_instance'].xcom_push(key='labeled_count', value=coverage['labeled_predictions'])
...
def train_shadow_model(**context):
    ...
    try:
        ...
        trigger_reason = context['task_instance'].xcom_pull(
            task_ids='check_retraining_needed',
            key='trigger_reason'
        ) or 'scheduled'
        ...
        X_train, X_eval, y_train, y_eval, validation_status = trainer.prepare_training_data_temporal(
...
def run_evaluation_gate(**context):
    ...
    try:
        ...
        is_first_deployment = context['task_instance'].xcom_pull(
            task_ids='evaluate_models_replay',
            key='is_first_deployment'
        )
        ...
        drifted_feature_names = drift_details.get('drifted_feature_names', []) if drift_details else []
...
with DAG(
    ...
    train_task = PythonOperator(
        task_id='train_shadow_model',
        python_callable=train_shadow_model,
        provide_context=True
    )
    ...
    gate_task = BranchPythonOperator(
        ...
        python_callable=run_evaluation_gate,
        provide_context=True
    )
    
    # Task 5a: Promote
    promote_task = PythonOperator(
        task_id='promote_shadow_model',
