...
def main():
    ...
    logger.info(f"✅ Default rate changed: {original_default_rate*100:.1f}% → {new_default_rate*100:.1f}%")
    ...
    for pred_id, true_label in zip(stats['prediction_ids'][:385], drift_data['SeriousDlqin2yrs'][:385]):
        label_store.store_label(
            prediction_id=pred_id,
            true_label=int(true_label),
            label_source="demo_population_shift"
        )
