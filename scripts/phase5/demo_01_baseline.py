...
def main():
    ...
    logger.info(f"Target distribution: {baseline_data['SeriousDlqin2yrs'].value_counts().to_dict()}")
    ...
    simulator = DataSimulator(
        ...
        delay_seconds=0.05  # 20 requests/second
    )
    ...
    for pred_id, true_label in zip(stats['prediction_ids'][:300], baseline_data['SeriousDlqin2yrs'][:300]):
        label_store.store_label(
            prediction_id=pred_id,
            true_label=int(true_label),
            label_source="demo_baseline"
        )
        labeled_count += 1
    ...
    logger.info("2. Verify no drift detected: cat /app/monitoring/reports/drift_reports/drift_summary_*.json | tail -1")
