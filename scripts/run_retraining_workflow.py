...
def main():
    ...
    logger.info(f"Using temporal windows: train_end={train_end.date()}, eval_start={eval_start.date()}, eval_end={eval_end.date()}")
    logger.info(f"  (70% train={split_idx} samples, 30% eval={len(sorted_preds)-split_idx} samples)")
    ...
    model, run_id = trainer.train_shadow_model(
        X_train, y_train,
        X_eval, y_eval,
        trigger_reason="manual_workflow"
    )
