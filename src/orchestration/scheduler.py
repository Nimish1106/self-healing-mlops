...
DESIGN PRINCIPLE: This is a DUMB scheduler.
- It knows WHEN to run jobs
- It does NOT know WHY or WHAT the jobs do
- It does NOT interpret results
- It does NOT make decisions

Think of this as a glorified cron job in Python.
For production, replace with Airflow/Prefect (Phase 4+).
"""
import time
import logging
...
class SimpleScheduler:
    ...
    NOT responsible for:
    - Deciding if job should run based on results
    - Handling job failures (beyond logging)
    - Complex scheduling (dependencies, DAGs)
    - Resource management
    
    ONLY responsible for:
    - Running job every N seconds
    ...
    def __init__(
        ...
        Args:
            ...
            job_function: Function to call (should be monitoring_job.run)
    ...
    def run_once(self) -> dict:
        ...
        Returns job results (but doesn't act on them).
        """
        ...
        try:
            ...
            # Log status (but don't interpret results)
...
def main():
    ...
    - MONITORING_INTERVAL: How often to run (seconds)
    - MONITORING_LOOKBACK: Time window for analysis (hours)
    """
    import os
    ...
    scheduler = SimpleScheduler(
        ...
        job_function=run_monitoring_job,
