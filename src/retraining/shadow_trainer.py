...
class ShadowModelTrainer:
    ...
    def prepare_training_data_temporal(
        ...
        merged = predictions_df.merge(
            labels_df[['prediction_id', 'true_label']],
            on='prediction_id',
            how='inner'
        )
        ...
        if duplicates_count > 0:
            logger.warning(f"⚠️  Found {duplicates_count} duplicate prediction_ids. Dropping duplicates (keeping last).")
        ...
        if num_classes < 2:
            validation_status["valid"] = False
            validation_status["message"] = f"Eval set is single-class: {num_classes} class(es) found"
    ...
    def train_shadow_model(
        ...
        with mlflow.start_run(
            run_name=f"shadow_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        ) as run:
            ...
            mlflow.log_param("data_validation_status", "valid" if validation_status["valid"] else "invalid")
            ...
            if not validation_status["valid"]:
                ...
                return None, run_id, {
                    "status": "aborted",
                    "reason": validation_status["message"],
                    "run_id": run_id
                }
            ...
            dataset_metadata = get_dataset_metadata(
                pd.concat([X_train, X_eval], axis=0)
            )
            ...
            metrics = evaluator.evaluate_predictions(
                y_eval, pd.Series(y_pred), pd.Series(y_prob)
            )
            ...
            if 'confusion_matrix' in metrics:
                ...
                if fp == 0 and fn == 0:
                    logger.warning("⚠️  Perfect predictions (no FP/FN). Verify no data leakage or duplication.")
