...
class EvaluationGate:
    ...
    def evaluate(
        ...
        logger.info(f"  F1 improvement: {f1_improvement_pct:.2f}% (min: {self.min_f1_improvement_pct}%)")
    ...
    def _check_promotion_cooldown(self) -> Tuple[bool, str]:
        ...
        promotion_files = sorted(
            self.decisions_path.glob("decision_*.json"),
            reverse=True
        )
            ...
            for segment, prod_metrics in prod_feature_segments.items():
                ...
                if prod_f1 > 0:
                    ...
                    if regression_pct < -self.max_segment_regression_pct:
                        issues.append(
                            f"{feature}={segment}: F1 dropped {abs(regression_pct):.1f}%"
                        )
