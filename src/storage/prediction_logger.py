...
def _repair_csv_header(csv_path: Path):
    ...
    try:
        ...
        logger.warning(f"⚠️  Cannot repair CSV header due to permission denied: {csv_path}. Data integrity may be affected until permissions are fixed. Error: {e}")
...
class PredictionLogger:
    ...
    def __init__(self, storage_path: str = "/app/monitoring/predictions/predictions.csv"):
        ...
        if self.storage_path.exists():
            ...
            if not is_valid:
                logger.warning(
                    f"⚠️  CSV header is misaligned. Repairing {self.storage_path}..."
                )
    ...
    def get_predictions_with_features(
        self,
