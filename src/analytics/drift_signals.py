...
class DriftSignalChecker:
    ...
    def check_drift_signals(self) -> Tuple[bool, Dict]:
        ...
        logger.info(
            f"Analyzing {len(recent_reports)} recent drift reports (last {self.lookback_hours}h)"
        )
    ...
    def _get_recent_reports(self) -> List[Dict]:
        ...
        logger.info(
            f"Filtered to {len(recent_reports)} reports within last {self.lookback_hours} hours"
        )
    ...
    def _parse_drift_report(self, report: Dict) -> Tuple[float, List[str]]:
        ...
        try:
            ...
            logger.debug(
                f"Parsed drift report: score={drift_score:.3f}, features={len(drifted_features)}"
            )
