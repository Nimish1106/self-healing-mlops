...
class ModelEvaluator:
    ...
    def replay_evaluation(
        ...
        prod_metrics = self.evaluate_predictions(
            y_true, pd.Series(prod_pred), pd.Series(prod_prob)
        )
        ...
        # Log comparison
        f1_diff = shadow_metrics['primary_metrics']['f1_score'] - prod_metrics['primary_metrics']['f1_score']
    ...
    def compare_models(
        ...
        comparison = {
            ...
            "f1_improvement_pct": (
                ((shadow_f1 - prod_f1) / prod_f1 * 100) if prod_f1 > 0 else 0.0
            ),
            "roc_auc_improvement": (
                shadow_primary.get('roc_auc', 0) - prod_primary.get('roc_auc', 0)
            ),
            "brier_change": (
                shadow_calib.get('brier_score', 1) - prod_calib.get('brier_score', 1)
            ),
